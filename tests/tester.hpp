...
public:
   action_result push_action_signup( const account_name& signer, const action_name &name, const variant_object &data ) {
      ...
      action act;
      act.account = N(cointreasury);
      act.name    = name;
      act.data    = signup_abi_ser.variant_to_binary( action_type_name, data,abi_serializer_max_time );
      return base_tester::push_action( std::move(act), uint64_t(signer));
   }

   action_result push_action_tok( const account_name& signer, const action_name &name, const variant_object &data ) {
   ...
   action_result transfer_tok( account_name from,
                  account_name to,
                  asset        quantity,
                  string       memo ) {
      return push_action_tok( from, N(transfer), mvo()
           ( "from", from)
           ( "to", to)
           ( "quantity", quantity)
           ( "memo", memo)
   ...
   action_result signup_claim( account_name from ) {
      return push_action_signup( from, N(claim), mvo()
           ( "account_name", from )
      );
   }

   action_result signup_refund( account_name signer ) {
      return push_action_signup( signer, N(refund), mvo() );
   }
   ...
   void basic_setup() {
      ...
      create_accounts({ N(eosio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake),
               N(eosio.bpay), N(eosio.vpay), N(eosio.saving), N(eosio.names), N(eosio.rex), N(cointreasury), N(cryptobank24) });
      ...
      //DEPLOY my code
      set_code( N(cointreasury), contracts::signup_wasm());
      set_abi( N(cointreasury), contracts::signup_abi().data() );
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(cointreasury) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         signup_abi_ser.set_abi(abi, abi_serializer_max_time);
      }
   }
